...
public:
...
#endif // MOVING_BOUNDARIES
#if defined(PARTICLES)&&!defined(FORCE_FIELD) /// еые ндмю опнакелю 
	void integrate_particles(const ulong steps=max_ulong); // intgegrate passive tracer particles forward in time in stationary flow field
	...
	void set_f(const float fx, const float fy, const float fz) { set_fx(fx); set_fy(fy); set_fz(fz); } // set global froce per volume
	void set_nu(const float nu) { for (uint d = 0u; d < get_D(); d++) lbm[d]->set_nu(nu); } // set kinematic shear viscosity
	...
	ulong iindex(const int x, const int y, const int z) const { // turn 3D coordinates into 1D linear index
		return (ulong)x + ((ulong)y + (ulong)z * (ulong)Ny) * (ulong)Nx;
	}
		...
		~Graphics() { // destructor must wait for all encoder threads to finish
			...
			while(last_value>0) {
				...
				if(last_value!=current_value) {
					print_info("Finishing encoder threads: "+loc::to_string(current_value));
