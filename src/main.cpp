...
#include "lbm.hpp"
#include "setup.hpp"
#include "setups/TVS_PZ.hpp"
//#include "setups/mercedes.hpp"
#include "setups/NN_MPI.hpp"
//#include "setups/Turb_chan.hpp"
...
void main_label(const double frametime) {
	if(info.allow_rendering) {
		...
		const int c = color(255-red(GRAPHICS_BACKGROUND_COLOR), 255-green(GRAPHICS_BACKGROUND_COLOR), 255-blue(GRAPHICS_BACKGROUND_COLOR));
		{
			...
			draw_label(ox, oy+i, "Resolution "     +alignr(26u, loc::to_string((info.lbm->get_Nx()))+"x"+loc::to_string(info.lbm->get_Ny())+"x"+loc::to_string(info.lbm->get_Nz())+" = "+loc::to_string(info.lbm->get_N())), c); i+=FONT_HEIGHT;
			...
			draw_label(ox, oy+i, "Kin. Viscosity " +alignr(22u, /************************************************************************************************/ loc::to_string(info.lbm->get_nu(), 8u)), c); i+=FONT_HEIGHT;
			draw_label(ox, oy+i, "Relaxation Time "+alignr(21u, /***********************************************************************************************/ loc::to_string(info.lbm->get_tau(), 8u)), c); i+=FONT_HEIGHT;
			draw_label(ox, oy+i, "Reynolds Number "+alignr(21u, /**********************************************************/ "Re < "+string(Re>=100.0f ? loc::to_string(to_uint(Re)) : loc::to_string(Re, 6u))), c); i+=FONT_HEIGHT;
			draw_label(ox, oy+i, "LBM Type "       +alignr(28u, /***************/ "D"+loc::to_string(info.lbm->get_velocity_set()==9u?2:3)+"Q"+loc::to_string(info.lbm->get_velocity_set())+" "+info.collision), c); i+=FONT_HEIGHT;
			draw_label(ox, oy+i, "Memory "         +alignr(30u, /*****/ "CPU "+loc::to_string(info.cpu_mem_required)+" MB, GPU "+loc::to_string(info.lbm->get_D())+"x "+loc::to_string(info.gpu_mem_required)+" MB"), c); i+=FONT_HEIGHT;
			draw_label(ox, oy+i, (info.steps==max_ulong ? "Elapsed Time   " : "Remaining Time ")+alignr(22u, /*************************************************************/ print_time(info.time())), c); i+=FONT_HEIGHT;
			draw_label(ox, oy+i, "Simulation Time "+alignr(21u, /***************************/ (units.si_t(1ull)==1.0f?loc::to_string(info.lbm->get_t()):loc::to_string(units.si_t(info.lbm->get_t()), 6u))+"s"), c); i+=FONT_HEIGHT;
		...
		if(!key_H) {
			...
			draw_label(ox, oy+i, "Scrollwheel or +/- ("+loc::to_string(camera.free ? (float)camera.free_camera_velocity : camera.zoom*(float)fmax(fmax(info.lbm->get_Nx(), info.lbm->get_Ny()), info.lbm->get_Nz())/(float)min(camera.width, camera.height), 3u)+"): zoom (centered camera mode) or camera movement speed (free camera mode)", c); i+=FONT_HEIGHT;
			...
			draw_label(ox, oy+i, "W/A/S/D/Space/C ("+loc::to_string(camera.pos.x/(float)info.lbm->get_Nx(), 2u)+"*Nx, "+loc::to_string(camera.pos.y/(float)info.lbm->get_Ny(), 2u)+"*Ny, "+loc::to_string(camera.pos.z/(float)info.lbm->get_Nz(), 2u)+"*Nz): move free camera", c); i+=2*FONT_HEIGHT;
			...
			draw_label(ox, oy+i, "N/M ("+loc::to_string(camera.eye_distance, 1u)+"): adjust eye distance for stereoscopic rendering", c); i+=2*FONT_HEIGHT;
...
#ifndef GRAPHICS
int main(int argc, char* argv[]) { // если вообще нет графики
